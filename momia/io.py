...
def measure_folder(dst_folder, drift_correction=True, profile_method='line',**kwargs):
    m = Data()
    m.load_cells_from_folder(dst_folder)
    if len(m.cells)>0:
        if drift_correction:
            m._correct_xy_drift(ref_channel=None)
        m.measure()
        m.data_df.to_csv('{}data.csv'.format(dst_folder))
        if profile_method == 'line':
            m.profile_midline()
            pk.dump(m.line_profiles, open('{}midline_profile.pk'.format(dst_folder),'wb'))
        elif profile_method == 'mesh':
            m.profile_orthogonal_mesh(**kwargs)
            pk.dump(m.mesh_profiles, open('{}orthogonal_profile.pk'.format(dst_folder), 'wb'))
        elif profile_method == 'both':
            m.profile_midline()
            pk.dump(m.line_profiles, open('{}midline_profile.pk'.format(dst_folder), 'wb'))
            m.profile_orthogonal_mesh(**kwargs)
            pk.dump(m.mesh_profiles, open('{}orthogonal_profile.pk'.format(dst_folder), 'wb'))